...
@section('content')
    ...
    <h3 class="admin-title mdui-text-color-indigo">创建新权限
        <button type="submit" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-pink-accent mdui-float-right"><i class="mdui-icon material-icons mdui-icon-left">edit</i>确认创建</button>
    </h3>
    @include('admin.layout.msg')
    <div class="mdui-typo-caption mdui-text-color-red mdui-m-t-1">警告:请谨慎对权限进行编辑修改等操作.</div>
    <form action="{{route('adminPermissionStore')}}" method="post">
        {{csrf_field()}}
        <div class="mdui-row">
        ...
        <button type="submit" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-pink-accent admin-btn"><i class="mdui-icon material-icons mdui-icon-left">edit</i>确认创建</button>
