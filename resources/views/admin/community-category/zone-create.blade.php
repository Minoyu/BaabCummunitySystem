...
@section('content')
        <form id="createCommunityZoneForm" method="post" action="{{route('adminCommunityZoneStore')}}">
            {{csrf_field()}}
            <h3 class="admin-title mdui-text-color-indigo">创建社区分区</h3>

            @include('admin.layout.msg')
            <div class="mdui-row">
                <div class="mdui-textfield mdui-textfield-floating-label mdui-col-xs-12 mdui-col-sm-10 mdui-col-md-6">
                    <h3 class="admin-index-title mdui-text-color-indigo">1.新分区名称</h3>
            ...
            <button onclick="formPublicSubmit('#createCommunityZoneForm')" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-pink-accent admin-btn"><i class="mdui-icon material-icons mdui-icon-left">add</i>发布</button>
            <button onclick="formHiddenSubmit('#createCommunityZoneForm')" class="mdui-btn mdui-btn-raised mdui-ripple admin-btn"><i class="mdui-icon material-icons mdui-icon-left">local_cafe</i>暂存</button>
