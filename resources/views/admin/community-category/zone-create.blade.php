...
@section('content')
        <form id="createCommunityZoneForm" method="post" action="{{route('adminCommunityZoneStore')}}">
            {{csrf_field()}}
            <h3 class="admin-title mdui-text-color-indigo">{{__('admin/community.createCommunityZone')}}</h3>
            ...
            <h3 class="admin-index-title mdui-text-color-indigo">3.{{__('admin/community.zoneCover')}}
                <br><small class="show-file-title-sub">{{__('admin/community.zoneCoverTip')}}</small></h3>
            <label for="newsCoverUploadInput">
                <img src="/imgs/zone_default.jpg" class="avatar mdui-hoverable zoneImg" style="width: 150px; height: 150px">
            </label>
            ...
            <button onclick="formPublicSubmit('#createCommunityZoneForm')" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-pink-accent admin-btn"><i class="mdui-icon material-icons mdui-icon-left">add</i>{{__('admin.publish')}}</button>
            <button onclick="formHiddenSubmit('#createCommunityZoneForm')" class="mdui-btn mdui-btn-raised mdui-ripple admin-btn"><i class="mdui-icon material-icons mdui-icon-left">local_cafe</i>{{__('community.save')}}</button>
